...
// 함수명: areAllNumbersGreaterThen
// 반환값: 모든 원소가 특정 숫자보다 크면 true, 아니면 false

function areAllNumbersGreaterThen(array, number) {
...
try {
  console.log(areAllNumbersGreaterThen([10, 20, 30], 5)); // true
  console.log(areAllNumbersGreaterThen([10, 20, 30], 15)); // false
} catch (error) {}
...
const getValuesByKey = (users, key) => {
...
function groupByKey(students, key) {
  const newObj = {};
  students.forEach((student) => {
    if (!newObj.hasOwnProperty(student[key])) {
      newObj[student[key]] = [];
    }
    newObj[student[key]].push(student);
  });
  return newObj;
}
