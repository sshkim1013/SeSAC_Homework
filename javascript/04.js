...
allTodosCompleted = originTodos.every((element) => {
  if (element["completed"] === IS_COMPLETED) {
    return true;
  }
});
...
/* 로직 코드 작성 */
completedCount = originTodos.filter((element) => {
  if (element["completed"] === IS_COMPLETED) {
    return true;
  }
});
...
// 출력 코드
console.log("완료된 할 일 개수:", completedCount.length);
...
/* 로직 코드 작성 */
sortedTodos = originTodos.sort((e1, e2) => {
