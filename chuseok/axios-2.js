...
// 함수이름 : getAllCartsResponse
// 요구사항 : 모든 장바구니 목록 조회(Get all carts) API를 호출하고 응답 객체를 출력한다

async function getAllCartsResponse() {
...
// 문제. 아래 요구사항을 해결하는 함수를 작성하고 함수를 실행한다.
// 함수이름 : getAllCartsResponseKeys
// 요구사항 : 모든 장바구니 목록 조회(Get all carts) API를 호출하고 응답 객체의 모든 속성명(key)을 출력한다

async function getAllCartsResponseKeys() {
  const response = await axios.get(`${BASE_URL}/carts`);
  const data = response["data"];
  const keys = Object.keys(data);
  console.log(keys);
}
getAllCartsResponseKeys();
...
async function getSingleCartFirstProduct(id) {
  const config = { url: `${BASE_URL}/carts/${id}` };
  const response = await axios(config);
  const data = response["data"];
  const products = data["products"];
  const firstProductId = products[0]["id"];

  const response2 = await axios.get(`${BASE_URL}/products/${firstProductId}`);
  const data2 = response2["data"];
  console.log(data2);
}
getSingleCartFirstProduct(1);
